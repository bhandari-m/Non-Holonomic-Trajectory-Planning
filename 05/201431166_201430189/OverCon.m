...
% assumed for over constrained at t = tc/2 = 2.5
tcb2 = 2.5;
xcb2 = 12;
ycb2 = 12;
...
Ax = [getB(t0,0) getB(t0,1) getB(t0,2) getB(t0,3) getB(t0,4) getB(t0,5);
    ...
    getBder(tc,0) getBder(tc,1) getBder(tc,2) getBder(tc,3) getBder(tc,4) getBder(tc,5);
    getB(tcb2,0) getB(tcb2,1) getB(tcb2,2) getB(tcb2,3) getB(tcb2,4) getB(tcb2,5)];
Wx = pinv(Ax)*Dx;
...
[Fcb20, Fcb21, Fcb22, Fcb23, Fcb24, Fcb25] = get_coeff(Wx(1),Wx(2),Wx(3),Wx(4),Wx(5),Wx(6),t0,tcb2,tf);

Dy = [K0; yf-y0; Kf; yc-y0 ; K_0 ; K_f ; ycb2-y0];
Ay = [getB(t0,0) getB(t0,1) getB(t0,2) getB(t0,3) getB(t0,4) getB(t0,5);
    ...
    getBder(t0,0) getBder(t0,1) getBder(t0,2) getBder(t0,3) getBder(t0,4) getBder(t0,5);
    getBder(tf,0) getBder(tf,1) getBder(tf,2) getBder(tf,3) getBder(tf,4) getBder(tf,5);
    Fcb20 Fcb21 Fcb22 Fcb23 Fcb24 Fcb25];
Wk = pinv(Ay)*Dy;
...
figure;
plot(time,X);
title('x vs t for Over Constrained');
xlabel('time');
ylabel('X');

figure;
plot(time,Y);
title('y vs t for Over Constrained');
xlabel('time');
ylabel('Y');

figure;
plot(time,theta);
title('theta vs t for Over Constrained');
xlabel('time');
ylabel('theta');

figure;
plot(time,Xdot);
title('Xdot vs t for Over Constrained');
xlabel('time');
ylabel('Xdot');

figure;
plot(time,Ydot);
title('Ydot vs t for Over Constrained');
xlabel('time');
ylabel('Ydot');

figure;
plot(X,Y);
title('Y vs X for Over Constrained');
xlabel('X');
ylabel('Y');